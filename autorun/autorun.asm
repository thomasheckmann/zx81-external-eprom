...
; MODIFIED TO COMPILE WITH: sjasmplus autorun.asm --raw=autorun.bin --lst=autorun.lst
; ===========================================================
; -------------------------
...
;; RESTART
L0361:  JP      NC,RST2        ; jump forward to INITIAL if D is zero 
                                ; to reset the system
...
NL:     LD      (HL),$76        ;HAS 25 N/L CHR
        ...
        JP      C,AUTORUN_BOOT  ;JP TO AUTO BOOT CODE

NEW1:   CALL    L149A           ; routine CLEAR sets $80 end-marker and the 
...
;
;
; ZX81 AUTORUN - based on work by Wilf Rigter
; - https://quix.us/timex/rigter/AutoBasic.html
; 
; uses:
; - ZX7 by Einar Saukas (https://spectrumcomputing.co.uk/entry/27996/ZX-Spectrum/ZX7)
; - ZX81 ROM Dissassembly (https://github.com/ZXSpectrumVault/rom-disassemblies/blob/master/Sinclair%20ZX81/Sinclair-ZX81.asm)
;
;
; code at address 0x2000 (normal shadow rom) jumpstarted by ROM
;
AUTORUN_BOOT:
; decompress ZX7 data to $4009
        ld      hl, compressed_p
        ld      de, $4009       ; load data here...
        call    decompress

; prepare sysvars to make sure ZX81 doesn't crash
        ld      (iy+$00),$ff    ; clear ERR_SP

        ld      a, $00
        ld      ($4006), a      ; set 'K' MODE

        ld      (iy+$01),$c0    ; RESET FLAG
        jp      $0f2b           ; Exit via SLOW (to $0676, LINERUN)

decompress:
; taken from official ZX7 Source Code
        include "./ZX7_SourceCode/asm/dzx7_standard.asm"
